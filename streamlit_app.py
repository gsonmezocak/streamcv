...
def main_app():
    ...
    tab1, tab2, tab3 = st.tabs(["🔍 Job Matcher", "💼 Job Management", "👤 My Profile"])
    ...
    with tab2:
        ...
        jobs = get_job_postings_with_vectors()
...
def login_page():
    ...
    with col2:
        st.markdown('<h1 class="login-title">🎯 AI CV Matcher</h1>', unsafe_allow_html=True)
        st.markdown('<p class="login-subtitle">Find your perfect job match with intelligent AI</p>', unsafe_allow_html=True)
    ...
    with st.container():
        st.markdown('<div class="login-metrics">', unsafe_allow_html=True)
        stat_col1, stat_col2, stat_col3 = st.columns(3)
        with stat_col1:
            st.metric(label="👥 Active Users", value=f"{total_users:,}")
        with stat_col2:
            st.metric(label="💼 Job Openings", value=f"{total_jobs:,}")
        with stat_col3:
            st.metric(label="📄 CVs Matched", value=f"{total_profiles:,}")
    ...
    st.markdown("### 🌟 Why Choose AI CV Matcher?")
    
    feat_col1, feat_col2, feat_col3 = st.columns(3)
    
    with feat_col1:
        st.info("🤖 **AI-Powered Matching**\n\nAdvanced AI analyzes your CV and finds the best job matches")
    
    with feat_col2:
        st.info("⚡ **Instant Results**\n\nGet matched with top jobs in seconds, not hours")
    
    with feat_col3:
        st.info("📊 **Detailed Analytics**\n\nSee strengths, weaknesses, and match scores for each job")
